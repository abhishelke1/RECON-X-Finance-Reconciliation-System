"""Order model for tracking order records."""
import uuid
...
class Order(BaseModel):
    """Represents an order record from Razorpay or external systems."""
    ...
    order_timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    raw_data: Mapped[dict | None] = mapped_column(JSON, nullable=True)

    __table_args__ = (
        UniqueConstraint("source_id", "source_system", name="uix_order_source"),
    )
